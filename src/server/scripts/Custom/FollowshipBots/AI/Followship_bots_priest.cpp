...
namespace FSB_PriestActionsSpells
{
    bool BotInitialCombatSpells(Creature* bot, uint32& _globalCooldownUntil, bool& _ownerWasInCombat, uint8& _appliedInitialCBuffs)
    {
        ...
        if (FSBUtilsSpells::CanCastNow(bot, now, _globalCooldownUntil) && !player->HasAura(SPELL_PRIEST_POWER_WORD_SHIELD) && _ownerWasInCombat && _appliedInitialCBuffs == 0)
        {
            bot->CastSpell(player, SPELL_PRIEST_POWER_WORD_SHIELD);
        ...
        else if (FSBUtilsSpells::CanCastNow(bot, now, _globalCooldownUntil) && !player->HasAura(SPELL_PRIEST_RENEW) && _appliedInitialCBuffs == 1)
        {
            bot->CastSpell(player, SPELL_PRIEST_RENEW);
            _globalCooldownUntil = now + 1500;
