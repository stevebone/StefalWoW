...
namespace FSBDeadmines
{
    ...
    void HandleVehicleCombatCheck(Creature* bot)
    {
        ...
        if (!FSBDungeon::IsBotInDungeon(bot) || bot->GetMapId() != FSBDungeon::Maps::Deadmines || bot->GetMap()->GetDifficultyID() != DIFFICULTY_HEROIC ||
        ...
        bool HandleFoeReaper5000(Creature* bot, Unit* target)
        {
            ...
            if (IsFoeReaperAOEActive(target))
            {
                if (!baseAI->GetDungeonData()->mechanicFlagC)
                {
                    ...
                    bot->GetMotionMaster()->MoveChase(target, ChaseRange(FSBDeadmines::Encounters::FoeReaper5000MinAOEDistance, 20.f));
                    TC_LOG_DEBUG("scripts.fsb.dungeon", "FSB: HandleFoeReaper5000 Bot {} - Harvest/Overdrive active, backing away to {} yd", bot->GetName(), FSBDeadmines::Encounters::FoeReaper5000MinAOEDistance);
                }
                return true;
            }
