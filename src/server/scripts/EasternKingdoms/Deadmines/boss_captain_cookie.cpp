...
class boss_captain_cookie : public CreatureScript
{
    public:
        ...
        {
            boss_captain_cookieAI(Creature* pCreature) : BossAI(pCreature, BOSS_CAPTAIN_COOKIE /*DATA_CAPTAIN*/)
            {
            ...
            void Reset() override
            {
                ...
                //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: Reset()");
            }

            void MoveInLineOfSight(Unit* who) override
            {
                if (instance->GetBossState(/*DATA_ADMIRAL*/ BOSS_ADMIRAL_RIPSNARL) != DONE)
                    return;
                ...
                //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: LOS()");
            }
            ...
            void JustEngagedWith(Unit* who) override
            {
                ...
                //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: Combat()");
            }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MOVE:
                            ...
                            //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: EVENT MOVE");
                            break;
                        case EVENT_CAULDRON_1:
                            if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                                DoCast(target, SPELL_CAULDRON, true);
                            events.RescheduleEvent(EVENT_CAULDRON_2, 2s);
                            //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: EVENT Cauldron");
                            break;
                        case EVENT_CAULDRON_2:
                            if (auto pCauldron = me->FindNearestCreature(NPC_CAULDRON, 20.0f))
                                me->GetMotionMaster()->MoveJump(1, { pCauldron->GetPositionX(), pCauldron->GetPositionY(), pCauldron->GetPositionZ() }, 5.0f, 10.0f);
                            events.RescheduleEvent(EVENT_THROW_FOOD, 3s);
                            //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: EVENT Caludron 2");
                            break;
                        case EVENT_THROW_FOOD:
                            if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                            {
                                DoCast(target, ThrowFoodSpells[urand(0, 11)]);
                                //TC_LOG_DEBUG("scripts.ai.core", "COOKIE: EVENT CAST FOOD");
                            }
                            //if (DoCastAOE(ThrowFoodSpells[urand(0, 11)]))
...
class npc_captain_cookie_good_food : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* pPlayer, Creature* pCreature)
        {
            ...
            if (!instance)
                return true;
            if (instance->GetBossState(/*DATA_CAPTAIN*/ BOSS_CAPTAIN_COOKIE) != IN_PROGRESS)
                return true;
